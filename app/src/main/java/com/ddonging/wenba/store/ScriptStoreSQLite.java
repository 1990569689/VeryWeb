...
package com.ddonging.wenba.store;
...
import com.ddonging.wenba.model.ScriptCriteria;
import com.ddonging.wenba.model.ScriptId;
import com.ddonging.wenba.model.ScriptRequire;
import com.ddonging.wenba.model.ScriptResource;
...
public class ScriptStoreSQLite implements ScriptStore {
	...
	private static class ScriptDbHelper extends SQLiteOpenHelper {
		...
		public Script[] selectScripts(ScriptId[] ids, Boolean enabled) {
			...
			if (ids != null || enabled != null) {
				...
				if (ids != null) {
					...
					selectionIdArgsArr = selectionArgs.toArray(new String[selectionArgs.size()]);
				}
				if (enabled != null) {
					if (ids != null) {
			...
			Map<ScriptId, List<String>> excludes = selectPatterns(TBL_EXCLUDE, selectionIdStr, selectionIdArgsArr);
			Map<ScriptId, List<String>> includes = selectPatterns(TBL_INCLUDE, selectionIdStr, selectionIdArgsArr);
			Map<ScriptId, List<String>> matches = selectPatterns(TBL_MATCH, selectionIdStr, selectionIdArgsArr);
			Map<ScriptId, List<ScriptRequire>> requires = selectRequires(TBL_REQUIRE, selectionIdStr, selectionIdArgsArr);
			Map<ScriptId, List<ScriptResource>> resources = selectResources(TBL_RESOURCE, selectionIdStr, selectionIdArgsArr);
			Cursor cursor = db.query(TBL_SCRIPT, COLS_SCRIPT, selectionStr, selectionArgsArr, null, null, null);
			...
			while (cursor.moveToNext()) {
				...
				String[] excludeArr = (exclude == null) ? null : exclude.toArray(new String[exclude.size()]);
				List<String> include = includes.get(id);
				String[] includeArr = (include == null) ? null : include.toArray(new String[include.size()]);
				List<String> match = matches.get(id);
				String[] matchArr = (match == null) ? null : match.toArray(new String[match.size()]);
				...
				ScriptRequire[] requireArr = (require == null) ? null : require.toArray(new ScriptRequire[require.size()]);
				List<ScriptResource> resource = resources.get(id);
				ScriptResource[] resourceArr = (resource == null) ? null : resource.toArray(new ScriptResource[resource.size()]);
				String content = cursor.getString(10);
				scriptsArr[i] = new Script(name, namespace, excludeArr, includeArr, matchArr, description, downloadurl, updateurl, installurl, icon, runat, unwrap == 1, version, requireArr, resourceArr, content);
				i++;
			}
			cursor.close();
			return scriptsArr;
			...
			try {
				...
				for (ContentValues fieldsExclude : fieldsExcludes) {
					if (db.insert(TBL_EXCLUDE, null, fieldsExclude) == -1) {
						Log.e(TAG, "Error inserting new script into the database (table " + TBL_EXCLUDE + ")");
						return;
					}
				}
		...
		public void deleteScript(ScriptId id) {
			db.beginTransaction();
			try {
				db.delete(TBL_SCRIPT, COL_NAME + " = ? AND " + COL_NAMESPACE + " = ?", new String[] { id.getName(), id.getNamespace() });
				db.setTransactionSuccessful();
			} finally {
				db.endTransaction();
			}
	...
	private static class ScriptCache {
		...
		private final LinkedHashMap<String, Script[]> urlScripts = new LinkedHashMap<String, Script[]>(
