...
public class FileManager {
    ...
    public boolean isMusic(String extension) {
        return "mp3".equals(extension) || "flac".equals(extension) || "aac".equals(extension) || "amr".equals(extension) || "wav".equals(extension) || "ogg".equals(extension);
    }
    ...
    public boolean isTable(String extension) {
        ...
        if (isApk(extension)) {
            ...
            try {
                if (Build.VERSION.SDK_INT >= 24){   //判断安卓系统是否大于7.0  大于7.0使用以下方法
                    Uri Uri = FileProvider.getUriForFile(context, "ddonging.fileprovider", file);  //转为URI格式  第二个参数为AndroidManifest.xml中定义的authorities的值
                    //添加这一句表示对目标应用临时授权该Uri所代表的文件
                    intent.setDataAndType(Uri, "audio/*");
                }
                else{
                    intent.setDataAndType(Uri.fromFile(file), "audio/*");//存储位置为Android/data/包名/file/Download文件夹
                }
                context.startActivity(intent);   //启动生命周期
            }catch (Exception e)
            {
                e.printStackTrace();
            }
            ...
            try {
                if (Build.VERSION.SDK_INT >= 24){   //判断安卓系统是否大于7.0  大于7.0使用以下方法
                    ...
                    intent.setDataAndType(Uri.fromFile(file), "video/*");//存储位置为Android/data/包名/file/Download文件夹
                }
                context.startActivity(intent);   //启动生命周期
            }catch (Exception e)
            {
            ...
            try {
                ...
                if (Build.VERSION.SDK_INT >= 24) {
                    Uri Uri = FileProvider.getUriForFile(context, "ddonging.fileprovider", new File(path));
                ...
                ToastUtil.Toast(context, "附件不能打开，请下载相关软件！");
            }
    ...
    private void install(String path) {
       ...
       // if (!file.exists()) return;
        ...
        if(!file.exists()) {
            ToastUtil.Toast(context,"文件不存在");
        }else
        {
            try {
                if (Build.VERSION.SDK_INT >= 24){   //判断安卓系统是否大于7.0  大于7.0使用以下方法
                    Uri apkUri = FileProvider.getUriForFile(context, "ddonging.fileprovider", file);  //转为URI格式  第二个参数为AndroidManifest.xml中定义的authorities的值
                    //添加这一句表示对目标应用临时授权该Uri所代表的文件
                    intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                    intent.setDataAndType(apkUri,type);
                }
                else{
                    intent.setDataAndType(Uri.fromFile(file), type);//存储位置为Android/data/包名/file/Download文件夹
                }
                context.startActivity(intent);   //启动生命周期
            }catch (Exception e)
            {
